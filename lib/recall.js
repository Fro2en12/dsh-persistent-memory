...
export function stripNoise(token) {
...
export function expandToken(token) {
    ...
    for (const group of SYNONYM_GROUPS) {
        if (group.some((w) => token.includes(w) || w.includes(token)))
            out.push(...group);
    }
    return [...new Set(out)];
}
...
export function tokenizeForSemantic(s) {
...
export function semanticOverlap(query, text) {
...
export function keySimilarity(a, b) {
    const norm = (s) => s.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, ' ').trim().split(/\s+/).filter(Boolean);
    const ta = norm(a);
    const tb = norm(b);
    if (!ta.length || !tb.length)
        return 0;
...
export function scoreItem(item, query, isFirstTurn, env) {
    if (!query)
        return 0;
    ...
    for (const token of tokens) {
        ...
        if (stripped.length < 2 && token.length < 8)
            continue;
        ...
        if (key.includes(token)) {
            ...
            if (synonyms.some((w) => key.includes(w)))
                score += 2;
            else if (synonyms.some((w) => tags.some((tag) => tag.includes(w))))
                score += 1;
        }
    ...
    if (!isFirstTurn) {
        ...
        if (SIGNAL_WORDS.some((word) => q.includes(word))) {
            const signalInItem = SIGNAL_WORDS.some((word) => key.includes(word) || value.includes(word) || tags.some((tag) => tag.includes(word)));
            if (signalInItem)
                score += 2;
        }
    }
...
export function lexicalHit(item, query) {
    const key = item.key.toLowerCase();
    const value = item.value.toLowerCase();
    const tags = item.tags.map((tag) => tag.toLowerCase());
...
export function pickRecallItems(items, query, limit, useFallback, isFirstTurn, hasImage, env) {
    ...
    if (env.rrfRecall && (!env.rrfFirstTurnOnly || isFirstTurn) && top.length === 0 && query) {
        ...
        if (ranked.length > 0)
            return ranked.slice(0, limit).map((e) => e.item);
    }
    // 首轮不启用 fallback，避免用"最近记忆"凑数
    if (!useFallback || isFirstTurn)
        return top;
...
export function fitBudget(items, budget, maxChars, sanitize, opts = {}) {
    const kept = [];
    let used = 0;
    ...
    for (const item of items) {
        ...
        if (used + cost > budget && !(opts.atLeastOne !== false && kept.length === 0))
            break;
        kept.push(item);
        used += cost;
    }
 ...
 */
export function fitByRenderedLength(items, budget, render) {
    let kept = items;
