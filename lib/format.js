...
export function formatRecall(items, all, maxChars) {
    const fitted = items;
...
export function formatLesson(items, maxChars) {
    const fitted = items;
    const lines = fitted.map((item) => {
        const cleaned = sanitizeValue(item.value);
...
export function buildRerankManifest(items) {
...
export function buildIndexBlock(rawItems, workspaceScopes, excludeKeys) {
