...
export function registerExtraction(ctx, deps) {
    const { valueMaxChars, withLock, readItems, writeItems, defaultScope, makeId, maxItems, autoExtract, autoExtractCooldownMs, turnBuffers, lastExtractAt, lastManualWriteAt, extractingSessions, extractCounters, isSubagentAgent, setBounded, } = deps;
    ...
    async function extractAndWrite(sid, dialogue) {
        ...
        if (!llm) {
            ctx.logger.debug('[mem] extract skipped: llm service unavailable');
            return;
        }
        ...
        if (!provider || !model) {
            ctx.logger.debug('[mem] extract skipped: no provider/model selected');
            return;
        }
        ...
        if (!m) {
            ctx.logger.info('[mem] extract: LLM reply contained no JSON object — nothing extracted');
            return;
        }
        let parsed;
        try {
            ...
            ctx.logger.info('[mem] extract: LLM reply JSON parse failed');
            return;
        }
        ...
        ctx.logger.info('[mem] extract: %d candidate(s), %d written, %d skipped%s', memories.length, written, skipped, droppedByCapacity > 0 ? ` (${droppedByCapacity} dropped: library at maxItems=${maxItems}; run memory_dream)` : '');
    }
    if (autoExtract) {
        ...
        ctx.on('agent/turn-stopping', (payload) => {
            try {
                ...
                if (extractingSessions.has(sid))
                    return; // 同会话不重入
                if (extractCounters.globalInFlight >= MAX_CONCURRENT_EXTRACT)
