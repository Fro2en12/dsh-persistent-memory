...
import { isCompletedMark, MAX_IMPORT_BYTES } from './write-ops.js';
export function registerCommands(ctx, deps, writeOps) {
    const { dataDir, dataFile, defaultScope, fullMaxChars, store, readItems, writeItems, withLock, searchItems, makeId, } = deps;
            ...
            case 'remember': {
                ...
                try {
                    ...
                    if (!r.changed && !r.created) {
                        ...
                        action = `已合并更新：${r.scope}/${r.mergedKey}（与新 key "${r.key}" 高度相似，未新建条目）`;
                    else if (r.created)
                        action = `已写入记忆：${r.scope}/${r.key}`;
                    else
                        action = `已更新记忆：${r.scope}/${r.key}`;
