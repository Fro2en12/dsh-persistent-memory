...
import { ageLabel, bigramJaccard, contentSimilarity, fitBudget, keySimilarity, lexicalHit, pickRecallItems, rrfRanking, semanticOverlap, truncate, } from './recall.js';
...
export function apply(ctx, config) {
    ...
    const autoCaptureDetail = config.autoCaptureDetail === 'full' ? 'full' : 'brief';
    ...
    function isSubagentAgent(agent) {
        if (!agent?.session)
            return true; // 拿不到 → 按子代理处理（fail-closed）
        try {
    ...
    async function importFileToStore(filePath, scope) {
        if (/^\\\\/.test(filePath)) {
