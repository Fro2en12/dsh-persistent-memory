...
export function apply(ctx, config) {
    ...
    function tokenizeForSemantic(s) {
    ...
    function semanticOverlap(query, text) {
    ...
    function rrfRanking(items, query) {
    ...
    function scoreItem(item, query, isFirstTurn) {
        if (!query)
            return 0;
        ...
        if (key.startsWith('task.')) {
            ...
            if (Number.isFinite(ageDays) && ageDays > taskTtlDays)
                score -= 3;
        }
    ...
    function lexicalHit(item, query) {
        const key = item.key.toLowerCase();
        const value = item.value.toLowerCase();
        const tags = item.tags.map((tag) => tag.toLowerCase());
    ...
    function pickRecallItems(items, query, limit, useFallback, isFirstTurn, hasImage) {
                ...
                if (idx >= 0) {
                    ...
                    const clash = items
                        ...
                        sim: item.scope === scope
                            ? contentSimilarity(item, { key, value, scope, tags: [], id: '', createdAt: now, updatedAt: now })
                            : 0,
                    }))
    ...
    function parseImportEntries(raw, filePath) {
    ...
    async function executeMemoryCommand(invocation) {
        ...
        switch (sub) {
            ...
            case 'import': {
                ...
                let entries;
                try {
                    entries = parseImportEntries(raw, arg);
                }
                catch (err) {
