...
import { fitBudget, fitByRenderedLength, pickRecallItems } from './recall.js';
...
export function registerPreStep(ctx, deps) {
    const { autoRecall, autoRecallLimit, autoRecallMaxChars, autoRecallBudgetChars, injectionBudgetChars, autoRecallMinScore, autoRecallRelativeFloor, autoRecallScope, autoRecallFallback, autoCapture, autoRecallOnce, autoRecallCooldownMs, rrfRecall, rrfFirstTurnOnly, synonymExpansion, taskTtlDays, autoRecallRerankMax, valueMaxChars, runtime, readItems, withLock, sessionInjections, persistInjectionState, makeId, setBounded, isSubagentAgent, currentWorkspaceScopes, } = deps;
            ...
            try {
                ...
                if (runtime.autoRecall) {
                    ...
                    if (items.length > 0 && (isRule || isRegret) && lessonAllowed) {
                        ...
                        const lessonFitted = fitByRenderedLength(fitBudget(fresh, lessonAvail, autoRecallMaxChars, sanitizeValue, { atLeastOne: false }).kept, lessonAvail, (xs) => formatLesson(xs, autoRecallMaxChars));
                ...
                if (runtime.autoRecall) {
                    ...
                    if (recallAllowed) {
                        ...
                        if (items.length > 0) {
                            ...
                            const recallFitted = fitByRenderedLength(fitBudget(recalledItems, recallAvail, autoRecallMaxChars, sanitizeValue, { atLeastOne: false }).kept, recallAvail, (xs) => formatRecall(xs, items, autoRecallMaxChars));
