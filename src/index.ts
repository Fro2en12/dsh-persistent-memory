...
export function apply(ctx: Context, config: Config): void {
  ...
  const autoCaptureDetail: 'brief' | 'full' = config.autoCaptureDetail === 'brief' ? 'brief' : 'full'
        ...
        if (runtime.autoRecall) {
          ...
          if (items.length > 0 && (isRule || isRegret) && lessonAllowed) {
            ...
            if (fresh.length > 0) {
              ...
              persistInjectionState()
              for (const item of fresh) lessonKeys.add(`${item.scope}/${item.key}`)
              changed = true
            }
          }
        }
  ...
  ctx.effect(() => ctx.tools.register(defineTool({
    ...
    async execute(args: { key: string; value: string; full?: string; links?: string[]; scope?: string; tags?: string[]; confirmed?: boolean; source?: string }, exec?: any) {
      ...
      return withLock(() => withConflictRetry(async () => {
        ...
        const allWarnings = clashWarning ? [...warnings, clashWarning] : warnings
        return { ok: true, key, scope, created: result.created, mergedKey: result.mergedKey, updatedAt: now, ...(allWarnings.length ? { warnings: allWarnings } : {}) }
      }))
    },
