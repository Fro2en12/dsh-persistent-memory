...
export function extractQuery(messages: unknown[]): { query: string; hasImage: boolean } {
...
export function regretSignal(query: string): boolean {
...
export function ruleScene(query: string): boolean {
  const q = query.toLowerCase()
...
export function isLessonLike(item: MemoryItem): boolean {
...
export function pickLessonItems(items: MemoryItem[], query: string, isRegret: boolean, isRule: boolean, limit: number): MemoryItem[] {
