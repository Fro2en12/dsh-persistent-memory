...
export const isCompletedMark = (value: string): boolean =>
...
// ── 写侧闸门共享实现（M14，v0.1.23）────────────────────────────────
// memory_set 工具与 /memory remember 命令共用同一套校验与写入路径：
// 审批门（用户亲自输入豁免）、子代理隔离、前缀白名单、tags ≤3、凭据闸门、
// value 截断归档、冲突重试。
...
export interface WriteOps {
  commitMemory: (input: CommitInput, exec?: any) => Promise<{ ok: true; key: string; scope: string; created: boolean; changed: boolean; mergedKey: string; updatedAt: string; warnings?: string[] }>
  importFileToStore: (filePath: string, scope: string, guard?: { exec?: any; fromUser?: boolean }) => Promise<{ imported: number; skipped: number; rejected: number; summary: string }>
  oneLineSummary: (value: string) => string
  defaultImportScope: () => string
}

export function createWriteOps(deps: MemoryDeps): WriteOps {
  const {
    defaultScope, approveOnSet, dedupeOnSet, valueMaxChars,
    fullMaxChars, maxItems, runtime, readItems,
    writeItems, withLock, lastManualWriteAt, makeId,
    normalizeTags, setBounded, isSubagentAgent,
    importAllowRoots,
  } = deps
  ...
  async function commitMemory(input: CommitInput, exec?: any): Promise<{
    ...
    if (prefix !== 'auth') {
      ...
      if (findCredentialMatch(body)) {
        throw new Error('memory_set: 检测到疑似明文凭据（token/secret/api key/密钥等）。凭据类记忆请用 auth.* 前缀（用户授权保留）；其他前缀一律只记指针（去哪查），不记明文。若内容只是提到了 token/密钥 这类词、并非真凭据，改写措辞即可，不要改用 auth.*（那会让该条只以掩码返回、且不再参与自动注入）。')
      }
    }
