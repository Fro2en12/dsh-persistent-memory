...
export function walk(obj: unknown, prefix: string, entries: ImportEntry[]): void {
  if (Array.isArray(obj)) { obj.forEach((v, i) => walk(v, `${prefix}-${i}`, entries)); return }
  if (obj && typeof obj === 'object') {
    ...
    if (typeof o.value === 'string') {
      ...
      for (const [k, v] of Object.entries(o)) walk(v, `${prefix}.${slugKey(k)}`, entries)
    }
  }
}
