...
function prefixByContent(content: string): string {
  if (/必须|不要|禁止|一律|规则|默认|优先|纠正/i.test(content)) return 'rule'
  if (/教训|坑|切记|注意/i.test(content)) return 'lesson'
  return 'ref'
}
 ...
 */
export function importPrefixFor(sourcePrefix: string, content: string): string {
  if (KEY_PREFIX_WHITELIST.includes(sourcePrefix)) return sourcePrefix
...
function uniqueKey(key: string, seen: Map<string, number>): string {
 ...
 */
export function walk(obj: unknown, prefix: string, entries: ImportEntry[], depth = 0, seen?: Map<string, number>): void {
  if (depth > MAX_DEPTH) return
  // 数组序号用 '.' 拼接：'-0' 会让 key 首段变成 'ref-0'（不在白名单）→ 导入条目事后无法 memory_set
  if (Array.isArray(obj)) { obj.forEach((v, i) => walk(v, `${prefix}.${i}`, entries, depth + 1, seen)); return }
  if (obj && typeof obj === 'object') {
    ...
    if (typeof o.value === 'string') {
      ...
      for (const [k, v] of Object.entries(o)) walk(v, `${prefix}.${slugKey(k)}`, entries, depth + 1, seen)
    }
  }
}
 ...
 */
export function parseImportEntries(raw: string, filePath: string, opts: { valueMaxChars: number }): ImportEntry[] {
